regenerated /mnt/c/Users/USER/workspace/kern/docs/tool-catalog.md from the live MCP catalog
ter adding, renaming, or changing an MCP tool.
     The catalog:doc gate (G36) fails when this file is stale or a tool is missing. -->
# MCP Tool Catalog

Every model-facing tool the kern MCP server registers (`kern_*`). 143 tools.
...
| `kern_fts_search` | explore | low | FTS5 full-text search (#3) over the SQLite symbol index. Supports MATCH syntax ('greet', 'func AND greet', `file:"main.go"`). Requires a build with -tags sqlite and a persisted index. Falls back to a clear error on the default build. |
