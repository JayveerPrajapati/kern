...
// TestG28_ToolListIncludesRepairGuidance wires the in-process MCP server (the
// same engine the legacy blueprint-mcp binary served over stdio) and verifies
// blueprint_repair_guidance is advertised in the MCP tools/list response, then
// exercises the tool through the real JSON-RPC tools/call surface with a
// sample finding and asserts the structured repair contract.
func TestG28_ToolListIncludesRepairGuidance(t *testing.T) {
	if testing.Short() {
		t.Skip("E2E gate test — full pipeline; runs in nightly non-short suite")
	}
	// The standalone blueprint-mcp binary was declared redundant: serve the
	// same engine in-process over os.Pipe() stdio.
	srv := g5NewServer()
	...
	out := g5RunServerPipe(t, srv, input, "")
