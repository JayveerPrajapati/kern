...
func (ValidateStagedHandler) InputSchema() map[string]any {
...
// buildValidationService assembles the full change-validation service shared
// by the staged and proposed validation handlers: the arch/gitleaks/jscpd
// checks (detection delegated to the incumbents, each falling back to its
// in-house check when the binary is absent — T2.1), the policy engine, and
// the audit writer. When the repo has existing metrics they are attached so
// the validation updates them.
func buildValidationService(client *kern.KernClient, cfg *policy.LoadedConfig, kernVersion string, absRepo string) *service.BlueprintService {
...
func (h ValidateStagedHandler) Handle(ctx context.Context, args json.RawMessage) ToolResult {
	...
	svc := buildValidationService(client, cfg, kernVersion, absRepo)
...
func (ValidateProposedHandler) InputSchema() map[string]any {
	return map[string]any{
		"type": "object",
		"properties": map[string]any{
			"repo": map[string]any{
			...
			"context_provenance": map[string]any{
				"type":        "object",
...
func (ExplainFindingHandler) InputSchema() map[string]any {
	return map[string]any{
		"type": "object",
		"properties": map[string]any{
...
func (h ExplainFindingHandler) Handle(ctx context.Context, args json.RawMessage) ToolResult {
	...
	// Omit empty fields: a minimal finding (rule id only) must not render
	// hollow "Severity: / File: :0 / Message: " lines — absent data renders
	// as absent, matching the optional-field style of Explanation/Fix.
	if f.Severity != "" {
...
func (RepairGuidanceHandler) InputSchema() map[string]any {
	return map[string]any{
		"type": "object",
		"properties": map[string]any{
			"finding": map[string]any{
...
func evidenceList(evidence []domain.Evidence) []map[string]any {
