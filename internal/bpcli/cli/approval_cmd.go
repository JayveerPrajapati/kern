...
func runRequestApproval(args []string) int {
	...
	// Suppress the flag package's raw parse-error dump and its default usage
	// so a bad flag produces a single "blueprint: flags: ..." line + rc=2,
	// consistent with the shared parser's error style.
	fs.SetOutput(io.Discard)
	fs.Usage = func() {}
	if err := fs.Parse(args); err != nil {
		if err == flag.ErrHelp {
			// -h/--help: reproduce the flag package's usage dump (suppressed
			// above by SetOutput) and exit clean, as before.
			fs.SetOutput(os.Stderr)
			fmt.Fprintf(os.Stderr, "Usage of request-approval:\n")
			fs.PrintDefaults()
			return 0
		}
		fmt.Fprintf(os.Stderr, "blueprint: flags: %v\n", err)
		return 2
	}
	...
	fmt.Printf(").\n")
	fmt.Printf("A human must run: kern approve %s\n", ar.ID)
	return 0
}
...
func runApprovalDecision(decision string, args []string) int {
	...
	// R-8 follow-up: mirror cmd/kern's recordBlueprintAuditDecision so the
	// bpcli reject/approve decision is also appended to the tamper-evident
	// .kern/audit governance chain (not just the blueprint JSONL trail +
	// pipeline commit/BLOCK link). Best-effort: never fails the decision.
	recordDecisionAudit(absRoot, id, who, decision == "approve", *reason)
...
// recordDecisionAudit appends a human approve/reject decision to the
// project's tamper-evident governance chain (.kern/audit). Entry shape and
// best-effort semantics mirror cmd/kern's recordBlueprintAuditDecision and
// governance.FileStore.recordAudit exactly (Action approve/reject, Resource
// "approval:<id>", Policy "approval", Result approved/denied), so `kern audit`
// and the cmd_audit.go render-layer dedupe see the same rows a task-gated
// approval produces.
func recordDecisionAudit(absRoot, id, approver string, approve bool, reason string) {
	action := "reject"
	result := "denied"
	if approve {
		action = "approve"
	...
	if err := l.AppendExternal(entry); err != nil {
		// Loud, non-blocking — same contract as FileStore.recordAudit: the
		// decision is already persisted; a missing chain entry is detectable
		// via `kern audit repair`.
		stdlog.Printf("blueprint %s: approval %s %s by %s NOT recorded in audit chain: %v", action, id, result, approver, err)
	}
}
