...
// kernTestBinary builds the merged kern binary (the standalone blueprint
// binary was declared redundant: kern check/fix/ci/... forward to the same
// internal/blueprint/cli implementation) ONCE per test-binary run and
// returns its path. Every build helper (buildBlueprint in g11_test.go,
...
var (
	kernTestBinOnce sync.Once
	kernTestBinPath string
	kernTestBinErr  error
)

func kernTestBinary(t *testing.T) string {
	t.Helper()
	kernTestBinOnce.Do(func() {
		root, err := findBlueprintRepoRoot()
		if err != nil {
			kernTestBinErr = err
			return
		}
		candidate := filepath.Join(root, "bin", "kern")
		if info, err := os.Stat(candidate); err == nil && !info.IsDir() {
			kernTestBinPath = candidate
			return
		}
		dir, err := os.MkdirTemp("", "kern-test-bin-")
		if err != nil {
			kernTestBinErr = fmt.Errorf("create test bin dir: %w", err)
			return
		}
		kernTestBinPath = filepath.Join(dir, "kern")
		cmd := exec.Command("go", "build", "-buildvcs=false", "-o", kernTestBinPath, "./cmd/kern")
		cmd.Dir = root
		if out, err := cmd.CombinedOutput(); err != nil {
			kernTestBinErr = fmt.Errorf("build kern: %v\n%s", err, out)
		}
	})
	if kernTestBinErr != nil {
		t.Fatal(kernTestBinErr)
	}
	return kernTestBinPath
}

// findBlueprintRepoRoot walks up from the working directory to the dir
// containing go.mod (the repo root). Unlike the per-helper walkers
