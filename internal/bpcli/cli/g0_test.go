...
func TestG0_BaselineBuildsAndVets(t *testing.T) {
	if testing.Short() {
		t.Skip("E2E gate test — full build and vet; runs in nightly non-short suite")
	}
	root := findRepoRoot(t)
...
func TestG0_ExitCodeContract(t *testing.T) {
	...
	// blueprint shim's 4 for unknown commands).
	if code := runCommandExit(t, binPath, "frobnicate"); code != 2 {
		t.Errorf("unknown command: exit = %d, want 2 (usage; kern's dispatch returns 2 for unknown commands)", code)
	}

	// version → exit 0, output contains "kern" (the merged binary prints
	// `kern <commit> (dev)` instead of `blueprint <version>`).
	versionOut := runCommand(t, binPath, "version")
	if !strings.Contains(versionOut, "kern") {
		t.Errorf("version output %q does not contain \"kern\"", versionOut)
	}

	// Invalid config: temp repo with garbage .blueprint/config.yaml plus a
	// committed file → `kern check -repo <repo>` → exit 3.
	dir := t.TempDir()
