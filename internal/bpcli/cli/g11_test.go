...
// buildBlueprint returns the suite-wide kern binary (built once per run;
// see kernTestBinary in g4_test.go).
func buildBlueprint(t *testing.T) string {
	t.Helper()
	return kernTestBinary(t)
}

// runCICommand runs `kern ci` against a repo and returns stdout, stderr, exit code, and artifact.
