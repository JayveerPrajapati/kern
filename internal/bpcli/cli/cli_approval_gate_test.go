...
func TestP13_ApprovalGateE2E(t *testing.T) {
	...
	// (stdlib flag stops at the first positional — regression guard). kern's
	// `approve` subcommand is kern-native (runApprove), so the blueprint
	// approval decision runs in-process via RunApprovalDecision.
	other := t.TempDir()
	appOut, appCode := runApprovalFrom(t, other, "approve", id, "--repo", dir, "--reason", "looks fine")
	if appCode != 0 {
	...
	if rejOut, rejCode := runApprovalFrom(t, other, "reject", rejID, "--repo", dir, "--reason", "not now"); rejCode != 0 {
	...
	// --repo is exercised).
	if out, code := runApprovalFrom(t, other, "approve", "apr-does-not-exist", "--repo", dir); code != 3 {
		t.Fatalf("approve unknown id exit=%d want 3 (operational error); output:\n%s", code, out)
	}
	if out, code := runApprovalFrom(t, other, "approve", id, "--repo", dir); code != 3 {
...
// runApprovalFrom runs the blueprint approval decision in-process with stdout
// captured. kern's `approve` and `reject` subcommands are kern-native
// (runApprove/runReject, governance-store based), not the blueprint approval
// flow, so the migrated test drives RunApprovalDecision directly — from a
// DIFFERENT working directory with --repo, preserving the interspersed-flag
// regression guard runCLIFrom exercised via the binary.
func runApprovalFrom(t *testing.T, cwd, decision string, args ...string) (string, int) {
	t.Helper()
	prev, err := os.Getwd()
	if err != nil {
	...
	stdout, _ := captureRun(t, func() {
		code = RunApprovalDecision(decision, args)
	})
	return stdout, code
}
