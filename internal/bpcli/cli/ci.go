...
import (
	...
	blueprintversion "github.com/JayveerPrajapati/kern/internal/bpreceipt/version"
...
func runValidationPipeline(valRoot, absRoot string, changes []domain.FileChange, client *kern.KernClient, kernVersion string, cfg *policy.LoadedConfig) (domain.ValidationResult, *audit.Writer) {
	req := domain.ChangeRequest{
		...
		Files:          changes,
		// authz: CI is a known non-human source; its identity is "ci".
		AgentID: "ci",
	}
...
// audit record (G1/). A ci receipt sealed without a chain binding would
...
func discoverDiffChanges(repoRoot, base, head string) ([]domain.FileChange, error) {
	...
	for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
		...
		// tool's own output — a self-inflicted BLOCK.
...
// .blueprint/suppressions.yaml, .blueprint/owners.yaml, and
