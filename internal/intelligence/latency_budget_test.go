...
// source set (the real internal/index and internal/intelligence packages,
...
// latencyFixtureRoot copies the real internal/index and internal/intelligence
...
// binary is the package dir (internal/intelligence), hence "../index".
...
func TestLatencyBudget(t *testing.T) {
	...
	queries := []struct {
		...
		{"RankedSearch(Build)", func() { intel.RankedSearch(ix, "Build", 10) }},
		{"RankedSearch(NewServer)", func() { intel.RankedSearch(ix, "NewServer", 10) }},
		{"RankedSearch(Load)", func() { intel.RankedSearch(ix, "Load", 10) }},
