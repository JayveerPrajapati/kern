...
func TestLoadKernPoliciesFallback(t *testing.T) {
	dir := t.TempDir()
...
policies:
  ...
  architecture: block
`
	if err := os.WriteFile(filepath.Join(kernDir, "policies.yaml"), []byte(content), 0o644); err != nil {
		t.Fatal(err)
	}

	loaded, err := Load(dir)
	if err != nil {
		t.Fatalf("Load(.kern/policies.yaml) failed: %v", err)
	}
	if loaded.Policy.Mode != "enforce" {
		t.Fatalf("Mode = %q, want enforce", loaded.Policy.Mode)
	}
	if loaded.Policy.Rules[domain.CategorySecret] != domain.EnforcementBlock {
		t.Fatalf("CategorySecret = %v, want EnforcementBlock", loaded.Policy.Rules[domain.CategorySecret])
	}
}
