...
type LoadedConfig struct {
	...
	// Warnings collects non-fatal loader notices — e.g. a suppression
...
// missing suppressions/owners file is simply empty. Malformed or
...
func Load(repoRoot string) (*LoadedConfig, error) {
	...
	// suppression maturity: suppressions + owner routing are layered on
...
func findExistingFile(repoRoot string, candidates ...[]string) (string, []byte, error) {
...
// loadConfig reads and validates .blueprint/config.yaml or .kern/policies.yaml / .kern/config.yaml.
func loadConfig(repoRoot string) (*LoadedConfig, error) {
	path, data, err := findExistingFile(repoRoot,
		[]string{".blueprint", "config.yaml"},
		[]string{".kern", "policies.yaml"},
		[]string{".kern", "config.yaml"},
	)
	if err != nil {
		if errors.Is(err, fs.ErrNotExist) {
...
// loadSuppressions reads .blueprint/suppressions.yaml or .kern/suppressions.yaml
// and resolves it into Policy.Suppressions. A missing file is empty with no error;
// a malformed file or an invalid entry is a hard error, same style as config.yaml.
// An unknown rule id is a non-fatal warning surfaced via LoadedConfig.Warnings.
func loadSuppressions(repoRoot string, cfg *LoadedConfig) error {
	path, data, err := findExistingFile(repoRoot,
		[]string{".blueprint", "suppressions.yaml"},
		[]string{".kern", "suppressions.yaml"},
	)
	if err != nil {
		if errors.Is(err, fs.ErrNotExist) {
			return nil
...
// loadOwners reads .blueprint/owners.yaml or .kern/owners.yaml and resolves it into
...
func loadOwners(repoRoot string, cfg *LoadedConfig) error {
	path, data, err := findExistingFile(repoRoot,
		[]string{".blueprint", "owners.yaml"},
		[]string{".kern", "owners.yaml"},
	)
	if err != nil {
		if errors.Is(err, fs.ErrNotExist) {
			return nil
...
// best-effort "unknown rule" warning — a warning, never an error,
