...
// primaryStorePath returns the on-disk store a default-build Save writes: the
// SQLite store when compiled in, the JSON cache under -tags nosqlite. The
// EnsureFresh tests use it to observe "was the index rewritten?" without
// depending on Save's write format (Save is SQLite-primary since the
// persistence consolidation).
func primaryStorePath(root string) string {
	if SQLiteEnabled() {
		return filepath.Join(root, ".kern", "index.sqlite")
	}
	return filepath.Join(root, ".kern", "index.json")
}
...
func TestEnsureFresh_Fresh(t *testing.T) {
	...
	idxPath := primaryStorePath(root)
	before, err := os.Stat(idxPath)
	if err != nil {
		t.Fatalf("stat %s: %v", idxPath, err)
	}
	...
	if res.FreshnessProof.Verdict != FreshnessFresh {
		t.Errorf("verdict = %q, want %q", res.FreshnessProof.Verdict, FreshnessFresh)
	}
	if !res.Built {
	...
	if err != nil {
		t.Fatalf("re-stat %s: %v", idxPath, err)
	}
	if !after.ModTime().Equal(before.ModTime()) {
		t.Errorf("%s rewritten on the fresh path (update issued despite fresh index)", idxPath)
	}
}
...
func TestEnsureFresh_RebuildsWhenStale(t *testing.T) {
	...
	if res.FreshnessProof.Verdict != FreshnessFresh {
		t.Errorf("final verdict = %q, want %q (strict post-save re-verify must observe fresh)", res.FreshnessProof.Verdict, FreshnessFresh)
	}
	if res.Symbols < 3 {
...
func TestEnsureFresh_NoIndex(t *testing.T) {
	...
	if res.FreshnessProof.Verdict != FreshnessFresh {
		t.Errorf("final verdict = %q, want %q", res.FreshnessProof.Verdict, FreshnessFresh)
	}
	if _, err := os.Stat(primaryStorePath(root)); err != nil {
		t.Errorf("index not persisted after full build: %v", err)
	}
}
...
func TestEnsureFresh_LegacyIndexWithoutTreeOID(t *testing.T) {
	...
	// The proof must come from the loose StatusReport fallback (recorded
	// TreeOID still empty), proving the fallback fired instead of the OID
	// probe.
	...
	if !after.ModTime().Equal(before.ModTime()) {
		t.Errorf("%s rewritten on the legacy-fresh path (update issued despite fresh content)", idxPath)
	}
}
...
func TestEnsureFresh_DecisiveStaleSkipsLooseCheck(t *testing.T) {
	...
	if res.FreshnessProof.Verdict != FreshnessFresh {
		t.Errorf("final verdict = %q, want %q (post-rebuild strict re-verify must observe fresh)", res.FreshnessProof.Verdict, FreshnessFresh)
	}
}
...
func TestStrictProbeDetectsMtimePreservingEdit(t *testing.T) {
	...
	if st.FreshnessProof.Verdict != FreshnessFresh {
		t.Fatalf("baseline strict verdict = %q, want %q", st.FreshnessProof.Verdict, FreshnessFresh)
	}
	...
	if st.FreshnessProof.Verdict != FreshnessStale {
		t.Errorf("strict verdict after mtime-preserving edit = %q, want %q (git apply regression: strict must catch content changes)", st.FreshnessProof.Verdict, FreshnessStale)
	}
	if !st.Stale {
