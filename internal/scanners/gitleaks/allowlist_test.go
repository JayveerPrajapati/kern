...
// production gap (e.g. internal/bpcli/mcp/g15_test.go).
func TestGitleaksAllowlistOnlyTestFixturePasses(t *testing.T) {
	chk := NewCheck(nil, WithBinary("gitleaks"), WithRunner(fakeGitleaksRunner(gitleaksReportForFiles("internal/bpcli/mcp/g15_test.go"))))
	cr, err := chk.Run(context.Background(), domain.ChangeRequest{
		RepositoryRoot: t.TempDir(),
		Files: []domain.FileChange{
			{Path: "internal/bpcli/mcp/g15_test.go", Op: domain.OpWrite, Content: "const secret = \"AKIA1234567890ABCDEF\"\n"},
		},
	})
	if err != nil {
		t.Fatalf("Run returned error: %v", err)
