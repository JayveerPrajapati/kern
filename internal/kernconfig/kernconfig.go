// Package kernconfig loads per-project kern configuration from .kern/kern.yaml.
// The file is optional — a missing or unreadable file silently yields an empty
// config (same graceful-degradation contract as .kern/config.json).
//
// Schema (YAML):
//
//	profiles:
