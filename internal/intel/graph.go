// The canonical knowledge graph: promotes the persisted v1 index into a
// domain.Graph, adding provenance and version metadata, and exposes the
// unified query APIs (who calls X, what X depends on, what depends on X, and
// the affected APIs/services/events/tests). Graph wraps domain.Graph so the
// query methods hang off the canonical type while keeping full field access.
package intel
...
type Graph struct {
	...
	// testsByPkg maps a package directory (filepath.Dir of a symbol's file)
	// to the node IDs of the test symbols defined there. It backs the scoped
	// WhatTestsCover query so same-package test attribution never re-scans
	// the node set per call. Built lazily once per graph and guarded by
	// testsOnce; the graph is read-only after construction (FromIndex /
	// NewWithGraph), so the cache never goes stale — it is invalidated
	// exactly like the rest of the graph caches, i.e. when the index is
...
func FromIndex(ix *index.Index) Graph {
	...
	// both become node "Save"). graphPackagePathByFile maps each file to its package
...
// graphPackagePathByFile maps each file in the index's package table to its package
...
func graphPackagePathByFile(ix *index.Index) map[string]string {
