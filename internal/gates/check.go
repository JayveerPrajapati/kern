// The approval gate check (P1.3) lives in this package alongside the gate
// registry: it slots into the validation pipeline as a standard service.Check
...
func NewCheck(store *Store, cfg risk.Config) *Check {
...
func (c *Check) Run(ctx context.Context, req domain.ChangeRequest) (domain.CheckResult, error) {
	...
	if err != nil || ar.Status != StatusApproved {
		...
		if err == nil {
			...
			case StatusPending:
				detail = fmt.Sprintf("request %s is still pending; a human must run `kern approve %s`", approvalID, approvalID)
			case StatusExpired:
