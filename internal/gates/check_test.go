...
func newCheck(t *testing.T) (*Check, *Store) {
	t.Helper()
...
func TestBlockRejectedApproval(t *testing.T) {
	chk, store := newCheck(t)
	if err := store.Create(Request{ID: "apr-rej", RepoRoot: "/tmp/repo", Intent: "x", RiskLevel: "high", Requester: "agent-1", CreatedAt: time.Now()}); err != nil {
		t.Fatalf("Create: %v", err)
	}
...
func TestPassApproved(t *testing.T) {
	chk, store := newCheck(t)
	if err := store.Create(Request{ID: "apr-ok", RepoRoot: "/tmp/repo", Intent: "update boundaries", RiskLevel: "high", Requester: "agent-1", CreatedAt: time.Now()}); err != nil {
		t.Fatalf("Create: %v", err)
	}
