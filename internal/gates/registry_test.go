...
var (
	testFilesOnce   sync.Once
	cachedTestFiles map[string]string
	cachedFileFuncs map[string]map[string]bool
)
...
func skipDir(name string) bool {
	...
	switch name {
	case "bin", "vendor", "node_modules", "dist", "build", "sdk", "python", "docs", "homebrew", "graphify-out", "testfixture":
		return true
	}
	return false
}
...
func collectTestFiles(t *testing.T, root string) (map[string]string, map[string]map[string]bool) {
	t.Helper()
	testFilesOnce.Do(func() {
...
func TestRegistryForwardMapping(t *testing.T) {
	root := repoRoot(t)
	_, fileFuncs := collectTestFiles(t, root)
...
func TestRegistryReverseMapping(t *testing.T) {
	root := repoRoot(t)
	testFiles, fileFuncs := collectTestFiles(t, root)
