...
func (c *Check) Run(ctx context.Context, req domain.ChangeRequest) (domain.CheckResult, error) {
	...
	scen, err := LoadAll(req.RepositoryRoot)
	if err != nil {
	...
	info := DetectRepoInfo(req.RepositoryRoot)
	var applicable []Scenario
	for _, s := range scen {
...
// repoSandbox implements Sandbox by running commands directly in the
...
func (s repoSandbox) Run(ctx context.Context, repoRoot string, command []string) SandboxResult {
	if len(command) == 0 {
		return SandboxResult{Output: "no command specified", ExitCode: -1}
	}
	dir := repoRoot
	if dir == "." || dir == "" {
		dir = s.root
