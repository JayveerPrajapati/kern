...
func TestG9_BaselinePasses(t *testing.T) {
	if testing.Short() {
		t.Skip("runs a real sandboxed go test execution; covered by the deep nightly (blueprint-nightly.yml runs resilience without -short)")
	}
...
func TestG9_InjectedTimeoutActuallyInjected(t *testing.T) {
	if testing.Short() {
		t.Skip("waits on a real hanging server (2s read deadline); covered by the deep nightly")
	}
...
func TestG9_ResilientImplPasses(t *testing.T) {
	if testing.Short() {
		t.Skip("runs a real sandboxed go test with a live fault server (up to 10s); covered by the deep nightly")
	}
	dir := makeRepo(t, map[string]string{
		"main.go":      resilientHTTPClient,
		"main_test.go": resilientTestSuite,
...
func TestG9_NonResilientImplFails(t *testing.T) {
	if testing.Short() {
		t.Skip("runs a real sandboxed go test against a hanging server (8s sandbox timeout); covered by the deep nightly")
	}
	dir := makeRepo(t, map[string]string{
...
func TestG9_FailuresRepeatable(t *testing.T) {
	if testing.Short() {
		t.Skip("runs two real sandboxed go test executions against a hanging server (~16s); covered by the deep nightly")
	}
	dir := makeRepo(t, map[string]string{
		"main.go":      nonResilientHTTPClient,
		"main_test.go": nonResilientTestSuite,
