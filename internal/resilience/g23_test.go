...
func TestG23_ScenarioEndToEnd(t *testing.T) {
	if testing.Short() {
		t.Skip("runs the full check against a live fault server + generated go test (~18s); covered by the deep nightly")
	}
...
func TestG23_ResilientRepoPasses(t *testing.T) {
	if testing.Short() {
		t.Skip("runs the full check with a live fault server + generated go test (~11s); covered by the deep nightly")
	}
